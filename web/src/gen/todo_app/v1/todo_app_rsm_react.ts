   ...
   response: ListTodoListsResponse | undefined;
    ...
    mutations: {
      ...
      Promise<__resembleResponseOrError<AddTodoListResponse>>;
       ...
       optimistic_metadata?: any ) =>
      Promise<__resembleResponseOrError<DeleteTodoListResponse>>;
...
export const TodoLists = ({ id, storeMutationsLocallyInNamespace}: SettingsParams): TodoListsApi => {
  ...
  const AddTodoList = async (partialRequest: __bufbuildProtobufPartialMessage<AddTodoListRequest> = {}) => {
    ...
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.AddTodoList", "POST"));
  ...
  const ListTodoLists = async (partialRequest: __bufbuildProtobufPartialMessage<ListTodoListsRequest> = {}) => {
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.ListTodoLists", "POST"));
...
function hasRunningMutations(): boolean {
    ...
    const runningDeleteTodoListMutations = useRef<__resembleMutation<DeleteTodoListRequest>[]>([]);
    const recoveredDeleteTodoListMutations = useRef<
      [__resembleMutation<DeleteTodoListRequest>, () => void][]
    >([]);
    ...
    const [failedDeleteTodoListMutations, setFailedDeleteTodoListMutations] = useState<
      __resembleMutation<DeleteTodoListRequest>[]
    >([]);
    const queuedDeleteTodoListMutations = useRef<[__resembleMutation<DeleteTodoListRequest>, () => void][]>(
      []
    );
    const recoverAndPurgeDeleteTodoListMutations = (): [
      ...
      localStorage.removeItem(localStorageKeyRef.current);
      const mutations: __resembleMutation<DeleteTodoListRequest>[] = JSON.parse(value);
    ...
    const [unobservedPendingDeleteTodoListMutations, setUnobservedPendingDeleteTodoListMutations] =
      useState<__resembleMutation<DeleteTodoListRequest>[]>([]);
    ...
    async function __DeleteTodoList(
      mutation: __resembleMutation<DeleteTodoListRequest>
    ): Promise<__resembleResponseOrError<DeleteTodoListResponse>> {
      try {
        // Invariant that we won't yield to event loop before pushing to
    ...
    async function _DeleteTodoList(mutation: __resembleMutation<DeleteTodoListRequest>) {
      setUnobservedPendingDeleteTodoListMutations(
        (mutations) => [...mutations, mutation]
      )
      ...
      if (
        ...
        const deferred = new __resembleReactDeferred<__resembleResponseOrError<DeleteTodoListResponse>>(() =>
        ...
        // Maybe add to localStorage.
        __resemblePushMutationMaybeToLocalStorage(localStorageKeyRef.current, "DeleteTodoList", mutation);
    ...
    async function DeleteTodoList(
      partialRequest: __bufbuildProtobufPartialMessage<DeleteTodoListRequest>, optimistic_metadata?: any
    ): Promise<__resembleResponseOrError<DeleteTodoListResponse>> {
      const idempotencyKey = uuidv4();
      const request = partialRequest instanceof DeleteTodoListRequest ? partialRequest : new DeleteTodoListRequest(partialRequest);
    ...
    async function __DeleteTodo(
      ...
      try {
        ...
        return _Mutation<DeleteTodoRequest, DeleteTodoResponse>(
          ...
          //
          // See also 'resemble/helpers.py'.
          "/todo_app.v1.TodoLists.DeleteTodo",
          mutation,
          mutation.request,
          mutation.idempotencyKey,
    ...
    async function __CompleteTodo(
      ...
      try {
        ...
        return _Mutation<CompleteTodoRequest, CompleteTodoResponse>(
          ...
          //
          // See also 'resemble/helpers.py'.
          "/todo_app.v1.TodoLists.CompleteTodo",
          mutation,
          mutation.request,
          mutation.idempotencyKey,
      ...
      const loop = async () => {
        await __resembleRetryForever(async () => {
          try {// Wait for any mutations to complete before starting to
            ...
            // mutations.
            if (runningAddTodoListMutations.current.length > 0 || runningDeleteTodoListMutations.current.length > 0 || runningAddTodoMutations.current.length > 0 || runningDeleteTodoMutations.current.length > 0 || runningCompleteTodoMutations.current.length > 0) {
  ...
  const DeleteTodoList = async (partialRequest: __bufbuildProtobufPartialMessage<DeleteTodoListRequest> = {}) => {
    const request = partialRequest instanceof DeleteTodoListRequest ? partialRequest : new DeleteTodoListRequest(partialRequest);
    const requestBody = request.toJson();
    // Invariant here is that we use the '/package.service.method' path and
    // HTTP 'POST' method (we need 'POST' because we send an HTTP body).
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.DeleteTodoList", "POST"));
  ...
  const AddTodo = async (partialRequest: __bufbuildProtobufPartialMessage<AddTodoRequest> = {}) => {
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.AddTodo", "POST"));
  ...
  const ListTodos = async (partialRequest: __bufbuildProtobufPartialMessage<ListTodosRequest> = {}) => {
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.ListTodos", "POST"));

    if (!response.ok && response.headers.has("grpc-status")) {
      const grpcStatus = response.headers.get("grpc-status");
      let grpcMessage = response.headers.get("grpc-message");
      throw new Error(
        `'todo_app.v1.TodoLists.ListTodos' for '${id}' responded ` +
          `with status ${grpcStatus}` +
          `${grpcMessage !== null ? ": " + grpcMessage : ""}`
      );
    } else if (!response.ok) {
      throw new Error(
        `'todo_app.v1.TodoLists.ListTodos' failed: ${response.body}`
      );
    }
...
function hasRunningMutations(): boolean {
  ...
  const DeleteTodo = async (partialRequest: __bufbuildProtobufPartialMessage<DeleteTodoRequest> = {}) => {
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.DeleteTodo", "POST"));
  ...
  const CompleteTodo = async (partialRequest: __bufbuildProtobufPartialMessage<CompleteTodoRequest> = {}) => {
    ...
    // See also 'resemble/helpers.py'.
    const response = await guardedFetch(
      newRequest(requestBody, "/todo_app.v1.TodoLists.CompleteTodo", "POST"));
...
async function _Mutation<
    Request extends
AddTodoListRequest    |DeleteTodoListRequest    |AddTodoRequest    |DeleteTodoRequest    |CompleteTodoRequest,
    Response extends    AddTodoListResponse    |    DeleteTodoListResponse    |    AddTodoResponse    |    DeleteTodoResponse    |    CompleteTodoResponse  >(
    path: string,
